...
fun main() {
    val passports: List<Passport> = parsePassport(input)
    puzzle7(passports)
    puzzle8(passports)
}

private fun puzzle8(passports: List<Passport>) {
    val validPassportCount = passports.filter { it.isValidNew() }
        .count()

    println("puzzle8: $validPassportCount")
}

private fun puzzle7(passports: List<Passport>) {
    val validPassportCount = passports.filter { it.isValid() }
        .count()

    println("puzzle7: $validPassportCount")
}

private fun parsePassport(rawPassports: String): List<Passport> {
    return rawPassports.split("\n\n")
        .asSequence()
        .map { it ->
            val attributePair = it.replace("\n", " ")
                .split(" ")
                .filter { attribute -> attribute.isNotBlank() }
                .map { attribute ->
                    val attributeKeyValue = attribute.split(":")
                    attributeKeyValue[0] to attributeKeyValue[1]
                }
                .toMap()
            Passport(AttributeMap(attributePair))
        }
        .toList()
}

private class Passport(attributes: AttributeMap) {
    val byr: String? by attributes
    val iyr: String? by attributes
    val eyr: String? by attributes
    val hgt: String? by attributes
    val hcl: String? by attributes
    val ecl: String? by attributes
    val pid: String? by attributes
    val cid: String? by attributes
...
private val byrValidator: (String?) -> Boolean = {
    numberRangeValidator(it, 1920, 2002)
}

private val iyrValidator: (String?) -> Boolean = {
    numberRangeValidator(it, 2010, 2020)
}

private val eyrValidator: (String?) -> Boolean = {
    numberRangeValidator(it, 2020, 2030)
}

private val hgtValidator: (String?) -> Boolean = {
    val cmValidator: (Int) -> Boolean = { cm -> cm in 150..193 }
    val inValidator: (Int) -> Boolean = { _in -> _in in 59..76 }
    val unitConverter: (String) -> (String) -> Int = { hgtWithUnit ->
        { unit -> hgtWithUnit.removeSuffix(unit).toInt() }
    }

    it.validate { value ->
        when {
            value.endsWith("cm") -> {
                cmValidator(unitConverter(value)("cm"))
            }
            value.endsWith("in") -> {
                inValidator(unitConverter(value)("in"))
            }
            else -> false
        }
    }
...
private val hclValidator: (String?) -> Boolean = { hcl ->
    hcl.validate { value ->
        value.matches(Regex("#[a-f|0-9]{6}+"))
    }
}

private val eclValidator: (String?) -> Boolean = { ecl ->
    ecl.validate { value ->
        value.matches(Regex("(amb)|(blu)|(brn)|(gry)|(grn)|(hzl)|(oth)"))
    }
}

private val pidValidator: (String?) -> Boolean = { pid ->
    pid.validate { value ->
        value.matches(Regex("\\d{9}+"))
    }
}

private fun String?.validate(exec: (String) -> Boolean): Boolean = this?.let(exec) ?: false

private fun numberRangeValidator(input: String?, startNumber: Int, endNumber: Int): Boolean {
    return input.validate { value ->
        try {
            value.toInt() in startNumber..endNumber
        } catch (e: Throwable) {
            false
        }
    }
...
private class AttributeMap(private val attributes: Map<String, String>) {
