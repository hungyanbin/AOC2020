...
fun main() {
    val passwords: List<Password> = parser(input)
    puzzle3(passwords)
    puzzle4(passwords)
}

private fun puzzle3(passwords: List<Password>) {
    val validPasswordCount = passwords.map(::validatePassword)
        .filter { it }
        .count()
    println("puzzle3: $validPasswordCount")
}

private fun puzzle4(passwords: List<Password>) {
    val validPasswordCount = passwords.map(::validatePasswordNew)
        .filter { it }
        .count()
    println("puzzle4: $validPasswordCount")
}

private fun validatePassword(password: Password): Boolean {
    val count = password.value.count { c -> c == password.letter }
    return count in password.number1..password.number2
}

private fun validatePasswordNew(password: Password): Boolean {
    val letter1 = password.value[password.number1 - 1]
    val letter2 = password.value[password.number2 - 1]
    return (letter1 == password.letter).xor(letter2 == password.letter)
}

fun parser(input: String): List<Password> {
    val lines = input.split("\n")
    return lines.map { line ->
        val data = line.split(" ")
        val range = data[0].split("-")
        val minNumber = range[0].toInt()
        val maxNumber = range[1].toInt()
        val letter = data[1][0]
        val value = data[2]
        Password(minNumber, maxNumber, letter, value)
    }
}

data class Password(val number1: Int, val number2: Int, val letter: Char, val value: String)

private val input = """13-15 c: cqbhncccjsncqcc
2-3 v: zvdvfd
