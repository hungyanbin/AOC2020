...
private fun executeProgram(program: MaskProgram, instructionRunner: (ComputerSystem, Memory) -> ComputerSystem): Long {
    val initSystem = ComputerSystem(null, hashMapOf())
    val result = program.instructions
            .fold(initSystem, { system, instruction ->
                instruction.fold(
                        left = { bits: Bits ->
                            // replace mask
                            ComputerSystem(bits, system.memoryHeap)
                        },
                        right = { memory: Memory ->
                            instructionRunner(system, memory)
                        }
                )
            })

    return result.memoryHeap
            .toList()
            .fold(0L, { acc, (_, value) -> acc + value })
}

private fun puzzle27(program: MaskProgram) {
    val answer = executeProgram(program, ::executeInstructionV1)
    println("puzzle 27: $answer")
}

private fun puzzle28(program: MaskProgram) {
    val answer = executeProgram(program, ::executeInstructionV2)
    println("puzzle 28: $answer")
}

private fun executeInstructionV1(system: ComputerSystem, memory: Memory): ComputerSystem {
    val maskBits = system.maskBits!!
    val valueInBits = maskBits.mapIndexed { index, maskBit ->
        val bitInMemory = memory.value
                .getBitAt(index)

        when (maskBit) {
            Bit.On -> 1
            Bit.Off -> 0
            Bit.Ignore -> bitInMemory
        }
    }

    val value = valueInBits.reduceRight { i: Long, acc: Long -> acc * 2 + i }
    val memoryHeap = system.memoryHeap.addOrReplace(memory.address.toLong(), value)

    return ComputerSystem(maskBits, memoryHeap)
}

private fun executeInstructionV2(system: ComputerSystem, memory: Memory): ComputerSystem {
    val maskBits = system.maskBits!!
    val address = memory.address.toMaskBits(maskBits.size)
    val applyMaskAddress = address.applyMask(maskBits)

    val ignoreCount = applyMaskAddress.count { it == Bit.Ignore }
    val replacements = listAllBitsCombinations(ignoreCount)

    val replaceAddresses = replacements.map { ignoredBits ->
        fillIgnoredBits(applyMaskAddress, ignoredBits).toLong()
    }

    val mutableMap = system.memoryHeap.toMutableMap()
    replaceAddresses.forEach { replaceAddress ->
        mutableMap[replaceAddress] = memory.value
    }
    return ComputerSystem(system.maskBits, mutableMap)
}

private fun Int.toMaskBits(length: Int): List<Bit> {
    val bits = this.toString(radix = 2)
            .map { char ->
                when (char) {
                    '0' -> Bit.Off
                    '1' -> Bit.On
                    else -> throw IllegalStateException("wrong char $char")
                }
            }.reversed()

    // Add 0 in the front
    return if (bits.size < length) {
        bits + List(length - bits.size) { Bit.Off }
    } else {
        bits
    }
}

// Replace the ignore bits with fillBits
// NOTE: Ignored bits size from inputBits can not larger than fillBits size
private fun fillIgnoredBits(inputBits: Bits, fillBits: Bits): Bits {
    val pair = inputBits.fold(Pair(listOf<Bit>(), 0), { (outputBits, fillIndex), nextBit ->
        when (nextBit) {
            Bit.On -> Pair(outputBits + Bit.On, fillIndex)
            Bit.Off -> Pair(outputBits + Bit.Off, fillIndex)
            Bit.Ignore -> Pair(outputBits + fillBits[fillIndex], fillIndex + 1)
        }
    })
...
// List all the possibilities for binary data
// for example:
// length = 3
// [Off, Off, Off]
// [Off, Off, On]
...
private fun listAllBitsCombinations(length: Int): Sequence<Bits> {
    val range = 0 until 2.0.pow(length).toInt()
    return range.asSequence()
            .map { number ->
                number.toMaskBits(length)
            }
}

private fun Bits.applyMask(maskBits: Bits): Bits {
    require(this.size == maskBits.size)
    return this.zip(maskBits) { value, maskBit ->
        when (maskBit) {
            Bit.On -> Bit.On
            Bit.Off -> value
            Bit.Ignore -> Bit.Ignore
        }
    }
}

private fun Bits.toLong(): Long {
    return this.foldRight(0L) { bit: Bit, acc: Long ->
        val i = when (bit) {
            Bit.On -> 1L
            Bit.Off -> 0L
            Bit.Ignore -> 0L
...
private fun Long.getBitAt(position: Int): Long {
    return (this shr position) % 2
}

private fun parseProgram(input: String): MaskProgram {
    val rawInstructions = input.split("\n")

    fun parseMaskBits(rawInstruction: String): Either<Bits, Memory> {
        val rawMask = rawInstruction
                .split(" ")
                .last()

        val maskBits = rawMask.map { c ->
            when (c) {
                'X' -> Bit.Ignore
                '0' -> Bit.Off
                '1' -> Bit.On
        ...
        return Either.Left(maskBits)
    }

    fun parseMemory(rawInstruction: String): Either<Bits, Memory> {
        val elements = rawInstruction.split(" ")
        val address = Regex("\\d+")
                .find(elements[0])!!
                .value.toInt()
        val value = elements.last().toLong()
        val memory = Memory(address, value)
        return Either.Right(memory)
    }

    val instructions = rawInstructions.map { rawInstruction ->
        if (rawInstruction.startsWith("mask")) {
            parseMaskBits(rawInstruction)
        } else if (rawInstruction.startsWith("mem")) {
            parseMemory(rawInstruction)
        } else {
            throw IllegalArgumentException("Illegal instruction $rawInstruction")
        }
    }

    return MaskProgram(instructions)
}

private class ComputerSystem(val maskBits: Bits?, val memoryHeap: Map<Long, Long>)

private class MaskProgram(val instructions: List<Either<Bits, Memory>>) {
    override fun toString(): String {
        return instructions.map {
            it.fold(
                    left = { maskBits -> "Mask: $maskBits\n" },
                    right = { memory -> "${memory}\n" }
            )
        }.toString()
    }
...
private typealias Bits = List<Bit>

private enum class Bit {
    On, Off, Ignore
}

private data class Memory(val address: Int, val value: Long)
