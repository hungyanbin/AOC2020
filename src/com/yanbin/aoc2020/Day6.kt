...
fun main() {
    val answerGroup = parseAnswers(input)
    puzzle11(answerGroup)
    puzzle12(answerGroup)
}

fun puzzle12(answerGroup: List<List<String>>) {
    val sumOfCount = countAnswer2(answerGroup.map { it.asSequence() }.asSequence())
    println("puzzle11: $sumOfCount")
}

private fun countAnswer2(answerGroup: Sequence<Sequence<String>>): Int {
    val everyoneAnswerSets = answerGroup.map { answers ->
        answers.drop(1)
            .fold(answers.first().toSet(), { set, answer ->
                set.flatMap { char ->
                    if (char in answer) {
                        listOf(char)
                    } else {
    ...
    return everyoneAnswerSets.map { it.size }
        .sum()
}

private fun puzzle11(answerGroup: List<List<String>>) {
    val sumOfCount = countAnswer(answerGroup)
    println("puzzle11: $sumOfCount")
}

private fun parseAnswers(answer: String): List<List<String>> {
    val groups = answer.split("\n\n")
    return groups.map { it.split("\n") }
}

private fun countAnswer(answerGroup: List<List<String>>): Int {
    val uniqueAnswerSets = answerGroup.map { answers ->
        answers.fold(HashSet<Char>(), { set, answer ->
            set.apply { addAll(answer.toList()) }
        })
    }

    return uniqueAnswerSets.map { it.size }
        .sum()
}
